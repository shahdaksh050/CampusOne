  ...
  return (
    <div className="page space-y-6">
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title">Attendance Map</h1>
          <p className="page-subtitle">View and analyze student attendance records</p>
        </div>
      </div>
      ...
      <Card className="glass-card">
        ...
        <CardContent>
          {summary.length === 0 ? (
            <div className="text-[var(--muted-foreground)]">No attendance records.</div>
          ) : (
            <div className="flex flex-col gap-2">
              {summary.map((s) => {
                ...
                return (
                  <div key={s.studentId} className="flex justify-between items-center p-3 border border-[var(--border)] rounded-md hover-lift bg-[var(--card)]/50">
                    <div className="flex flex-col">
                      <span className="font-medium">{s.name}</span>
                      <span className="text-sm text-[var(--muted-foreground)]">{s.present} present, {s.absent} absent, {s.late} late, {s.excused} excused</span>
                    </div>
